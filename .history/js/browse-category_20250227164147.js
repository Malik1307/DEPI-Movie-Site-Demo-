...
console.log(category+"asljd;kjdfksljhfk;hfjklhdfjkHDHAKJ");
...
<div class="movie-title">${
  category != "movie" ? movie.naem : movie.name
}</div>
</a>
`;
...
async function fetchingBrowseCategory() {
  console.log(category + "asljkfhsjldbfhjkfdhkaskl");

  const moviesGrid = document.querySelector(".movies_grid");
  moviesGrid.innerHTML = "";
  const data = await fetchMovies(
    `/discover/${category=="anime"?"tv":category}`,
    `${category == "anime" ? "&with_genres=16" : ""}`,
    1
  );
