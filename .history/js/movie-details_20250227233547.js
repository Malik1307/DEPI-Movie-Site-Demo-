...
async function fetchMovieDetails() {
  try {
    const response = await fetch(
      `${BASE_URL}/${
        isMovie ? "movie" : "tv"
      }/${movieId}?api_key=${API_KEY}&append_to_response=credits,recommendations,videos,revews`
    );
...
function updateReviewsSection(reviews, reviewsContent) {
  ...
  reviews.results.forEach((review) => {
    ...
    // Limit content length and add "Read more" option for long reviews
  ...
  setTimeout(() => {
    ...
    readMoreButtons.forEach(button => {
      button.addEventListener('click', function() {
        ...
        container.querySelector('.btn-read-less').style.display = 'inline-block';
      });
    });
    ...
    readLessButtons.forEach(button => {
      button.addEventListener('click', function() {
        const container = this.closest('.read-more-container');
        container.querySelector('.review-content-short').style.display = 'inline';
        container.querySelector('.review-content-full').style.display = 'none';
        this.style.display = 'none';
        container.querySelector('.btn-read-more').style.display = 'inline-block';
      });
    });
  }, 100);
}

// Add this to your updateMovieDetails function
function updateMovieDetails(movie) {
  // Keep all existing code
  
  // Near the end, add this section:
