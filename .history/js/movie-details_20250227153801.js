...
function updateMovieDetails(movie) {
  ...
  if (movieYear) {
    safelySetTextContent(movieYear, new Date(isMovie? movie.release_date:movie.first_air_date).getFullYear());
  }
  ...
  : movie.aggregate_credits.crew
      .filter(p => p.jobs.some(j => j.job === "Director"))
      .sort((a, b) => b.total_episode_count - a.total_episode_count)?.[0]?.name || "No director found";

safelySetTextContent(directorInfo, director || "Unknown");
      ...
      <div class="movie-card">
        <a href="movie-details.html?id=${movie.id}">
          ...
          <div class="movie-card-info">
            <h4 class="movie-card-title">${isMovie?movie.title:movie.name}</h4>
