  ...
  if (searchForm) {
    ...
    searchForm.addEventListener('submit', function(e) {
      ...
      if (searchQuery.length > 0) {
        ...
        const searchResultsUrl = `/pages/search-results.html?query=${encodeURIComponent(searchQuery)}`;
...
function createMovieOrShowItem(item, type) {
  ...
  resultItem.addEventListener('click', () => {
    window.location.href = `/pages/${type}-details.html?id=${item.id}`; // Updated to absolute path
  });
...
function createPersonItem(person) {
  ...
  resultItem.style.cursor = 'pointer';
  resultItem.addEventListener('click', () => {
    window.location.href = `/pages/person-details.html?id=${person.id}`; // Updated to absolute path
  });
