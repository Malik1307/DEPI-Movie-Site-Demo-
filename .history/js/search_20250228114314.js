    ...
    if (searchForm) {
      ...
      searchForm.addEventListener('submit', function(e) {
        ...
        if (searchQuery.length > 0) {
          ...
          // CRITICAL FIX: Define the exact path to search results page
          // This needs to match your folder structure exactly
          let searchResultsUrl = isSearchResultsPage 
            ? `search-results.html?query=${encodeURIComponent(searchQuery)}`
            : `pages/search-results.html?query=${encodeURIComponent(searchQuery)}`;
