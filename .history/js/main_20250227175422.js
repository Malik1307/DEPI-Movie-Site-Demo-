      ...
      <div class="carousel-caption">
        ...
        <div class="genres mt-2">
          ${movie.genre_ids.slice(0, 3).map(id => 
            `<div class="gcontent">${getGenreName(id, movie.media_type)}</div>`
          ).join('')}
        </div>
...
async function loadCarousel() {
  ...
  trendingItems.forEach((movie, index) => {
    ...
    itemDiv.innerHTML = `
      <img src="https://image.tmdb.org/t/p/original${movie.backdrop_path}" class="d-block w-100" alt="${movie.title || movie.name}">
      <div class="carousel-caption">
        ...
        <div class="genres mt-2">
          <div class="gcontent">${getGenres(movie.genre_ids)}</div>
        </div>
