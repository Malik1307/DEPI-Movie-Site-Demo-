...
// Add these 3 new functions (keep all existing code unchanged)
// ------------------------------------------------------------
...
function setupPlayButton(trailerKey) {
...
// Modify ONLY the initMovieDetails function as follows:
// -----------------------------------------------------
async function initMovieDetails() {
  console.log("Initializing movie details...");
  
  if (movieId) {
    // Existing movie details fetch
    const movieData = await fetchMovieDetails();
    if (movieData) updateMovieDetails(movieData);

    // New trailer handling (add this block)
    const videos = await fetchMovieVideos(movieId);
    const trailer = findBestTrailer(videos);
    if (trailer) setupPlayButton(trailer.key);
  ...
  // Keep existing UI initializations
  initTabs();
  initMoreButton();
}
...
function updateMovieDetails(movie) {
  ...
  if (moviePoster && movie.backdrop_path) {
    safelySetStyle(moviePoster, 'backgroundImage', `url(${BACKDROP_PATH}${movie.backdrop_path})`);
  }
  ...
  if (recommendedContent && movie.recommendations && movie.recommendations.results) {
    ...
    col.innerHTML = `
      <div class="movie-card">
        <a href="movie-details.html?id=${movie.id}">
          <img src="${movie.poster_path ? IMAGE_URL + movie.backdrop_path : 'placeholder-image.jpg'}" 
               alt="${movie.title}" 
