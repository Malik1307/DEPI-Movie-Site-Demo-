...
async function fetchMovieDetails() {
  try {
    const response = await fetch(`${BASE_URL}/tv/${movieId}?api_key=${API_KEY}&append_to_response=credits,recommendations`);
...
function updateMovieDetails(movie) {
  ...
  if (moviePoster && movie.poster_path) {
    safelySetStyle(moviePoster, 'backgroundImage', `url(${IMAGE_URL}${movie.poster_path})`);
  }
  
  if (movieBackdrop && movie.backdrop_path) {
    safelySetStyle(movieBackdrop, 'backgroundImage', `url(${BACKDROP_PATH}${movie.backdrop_path})`);
  }
