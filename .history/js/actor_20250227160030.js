...
const actorId = urlParams.get("id") || "17813"; // Default to Cillian Murphy if no ID provided

// MovieCard template function - same as your home page
const MovieCard = (movie, index, isMovie = 1) => ` 
<div class="col-6 col-sm-4 col-md-3 col-lg-2 mb-4">
  <a href="./movie-details.html?id=${movie.id}&media_type=${movie.media_type || 'movie'}" class="movie-card-link">
    <div class="movie-card">
      <div class="image-container">
        <img src="${movie.poster_path ? IMAGE_URL + movie.poster_path : '../assets/images/placeholder.jpg'}" alt="${
  movie.title || movie.name
}">
        <div class="overlay"></div>
        <div class="rating"><i class="fa-solid fa-star"></i> ${(movie.vote_average || 0).toFixed(1)}</div>
      </div>
      ${index < 10 ? '<div class="top-badge">TOP 10</div>' : ""}
    </div>
...
function updateMoviesGrid(movies) {
  ...
  movies.forEach((movie, index) => {
    row.innerHTML += MovieCard(movie, index);
  });
}
